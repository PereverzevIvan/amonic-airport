...
test_user_wrong_email = {"email": "wrong_email@amonic.com", "password": "123"}
...
class Auth:
    ...
    def login_user(self, do_assertion=True):
        response = requests.post(f"{self.api_url}/login", json=self.user_to_login)
    ...
    def refresh_user_tokens(self, do_assertion=True):
        response = requests.get(f"{self.api_url}/refresh", cookies=self.tokens_cookies)
    ...
    def logout_user(self, do_assertion=True):
        response = requests.get(f"{self.api_url}/logout", cookies=self.tokens_cookies)
    ...
    def __assert_refresh_response(self, response: requests.Response):
        ...
        old_tokens_version = jwt.decode(
            self.access_token, options={"verify_signature": False}
        )["ver"]
        new_tokens_version = jwt.decode(
            new_access_token, options={"verify_signature": False}
        )["ver"]
        assert (
            old_tokens_version + 1 + self.was_last_request_logout == new_tokens_version
        )
