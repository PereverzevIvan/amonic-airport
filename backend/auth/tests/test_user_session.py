...
def test_login_new_user_session(api_url):
    ...
    assert current_session.logout_at is None
...
def test_logout_user_session(api_url):
    ...
    assert login_session.logout_at is None
    ...
    assert logout_session.logout_at is not None
...
def test_login_2_times_to_check_invalid_logout_marked(api_url):
    ...
    login_session = user_sessions[0]
    assert (
        login_session.invalid_logout_reason is None
        or login_session.invalid_logout_reason == ""
    )
    ...
    assert invalid_logout_session.logout_at is None
...
def test_update_invalid_logout_session(api_url):
    ...
    assert new_invalid_logout_session.logout_at is None
