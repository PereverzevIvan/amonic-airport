...
def test_login_new_user_session(api_url):
    ...
    time_before_request = datetime_utils.now() 
    ...
    assert time_before_request <= current_session.login_at 
    time_after_request = datetime_utils.now(True)
    assert current_session.login_at <= time_after_request 
